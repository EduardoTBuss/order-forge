...
usage() {
  cat <<'EOF'
Usage: scripts/compose-clean-postgres.sh [--project <name>] [--compose-file <path>]

Removes the Postgres volume used by docker compose.
...
services=(postgres dbviewer)
volumes=("${project_name}_postgres_data")
